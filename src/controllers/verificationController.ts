...
export class VerificationController {
  static async createCustomer(req: Request, res: Response) {
    try {
      ...
      // Step 2: Optionally store customer in DIS (useful when endpoint is available)
      if (process.env.DIS_SKIP_STORE !== 'true') {
        try {
          ...
          if (error.message?.includes('404')) {
            console.warn(
              'DIS storeCustomer endpoint unavailable in this deployment; continuing without store'
            );
          } else {
            throw error;
          }
  ...
  static async uploadDocument(req: Request, res: Response) {
    try {
      ...
      const normalizedBack = backImage
        ? normalizeImagePayload(backImage)
        : undefined;
  ...
  static async performLivenessCheck(req: Request, res: Response) {
    try {
      ...
      if (!finalChallengeId) {
        const challenge = await innovatricsClient.createLivenessChallenge(
          customerId,
          {
      ...
      const livenessResult = await innovatricsClient.submitLivenessData(
        customerId,
        {
  ...
  static async compareFaces(req: Request, res: Response) {
    try {
      ...
      // Compare faces
      const similarityResult = await innovatricsClient.compareFaces(
        probeFaceId,
        {
          referenceFace,
  ...
  static async createLivenessChallenge(req: Request, res: Response) {
    try {
      ...
      return ResponseHandler.error(
        res,
        'Failed to create liveness challenge',
        500,
        error.message
      );
    }
